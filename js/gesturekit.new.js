...
GestureKit.prototype.init = function(options) {
    ...
    this.gk_container.addEventListener('touchstart', function (eve) {
        _points.length = 0;
        _isDown = 0;
        that.detectGesture(eve.touches);
    });
    ...
    this.gk_container.addEventListener('touchend', function () {
        var result = gk.kit._r.Recognize(_points);
...
GestureKit.prototype.detectGesture = function (touches) {
...
function gk(options) {
    gk.kit = gk.kit || new GestureKit(options);
    return gk;
}
